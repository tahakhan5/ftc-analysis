...
csv_file = open(os.path.join(OUT_PATH, '8_fraud_incomes.csv'), "w")
...
cursor.execute(
		'''
			SELECT CP.CustomerZip, CP.Crimes, CP.Population, incomes.MedianIncome FROM
			...
			INNER JOIN incomes ON incomes.ZipCode == CP.CustomerZip
	    ''')
